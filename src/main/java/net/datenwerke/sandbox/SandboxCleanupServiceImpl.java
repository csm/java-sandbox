...
/**
 * Default implementation of {@link SandboxCleanupService}.
 * 
...
public class SandboxCleanupServiceImpl implements SandboxCleanupService{
	...
	/*
	 * (non-Javadoc)
	 * @see net.datenwerke.sandbox.SandboxCleanupService#kill(net.datenwerke.sandbox.SandboxMonitoredThread, net.datenwerke.sandbox.jvm.exceptions.SandboxTaskKilledException)
	 */
	@Override
	public BadKillInfo kill(SandboxMonitoredThread monitor, SandboxedTaskKilledException exception) {
		boolean safe = true;
		BadKillInfo killInfo = null;
		
		SandboxedThread monitoredThread = monitor.getMonitoredThread();
		if(monitoredThread.isAlive()){
			monitoredThread.suspend();
			
			try{
				ThreadInfo threadInfo = threadBean.getThreadInfo(new long[]{monitoredThread.getId()}, true, true)[0];
			...
			monitoredThread.setKilled(safe, exception);
			monitor.getCallingThread().interrupt();
